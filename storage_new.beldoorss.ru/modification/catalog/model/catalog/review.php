...
class ModelCatalogReview extends Model {
	public function addReview($product_id, $data) {
		$this->db->query("INSERT INTO " . DB_PREFIX . "review SET author = '" . $this->db->escape($data['name']) . "', customer_id = '" . (int)$this->customer->getId() . "', product_id = '" . (int)$product_id . "', text = '" . $this->db->escape($data['text']) . "', rating = '" . (int)$data['rating'] . "', date_added = NOW()");
			...
			$new_review = $this->config->get('tlgrm_notification_new_review');
			...
			if ($send_ntf == 1 && $new_review) {
				...
				if (strlen($tlgrm_ntf_id) > 0) {
					...
					foreach ($tlgrm_ntf as $key) {
						$tlgrm_ntf[$key] = $this->config->get('tlgrm_notification_review_'.$key) || 0;
					}

					$this->load->language('extension/module/tlgrm_notification');

					$message = $this->language->get("text_review") . ' ' . $this->request->server['HTTP_HOST'] . PHP_EOL;
					...
					if ($this->config->get('tlgrm_notification_review_product_name')) {
