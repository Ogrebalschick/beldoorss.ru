...
class ModelAccountCustomer extends Model {
	public function addCustomer($data) {
		...
		$customer_id = $this->db->getLastId();
			...
			$new_customer = $this->config->get('tlgrm_notification_new_customer');
			...
			if ($send_ntf == 1 && $new_customer == 1) {
				...
				if (strlen($tlgrm_ntf_id) > 0) {
					...
					foreach ($tlgrm_ntf as $key) {
						$tlgrm_ntf[$key] = $this->config->get('tlgrm_notification_customer_'.$key) || 0;
					}

					$this->load->language('extension/module/tlgrm_notification');
	
					$message = $this->language->get("text_customer") . ' ' . $this->request->server['HTTP_HOST'] . PHP_EOL;
