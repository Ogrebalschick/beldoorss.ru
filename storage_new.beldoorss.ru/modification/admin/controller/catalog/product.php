...
public function getattributesbygroup() {
	...
	$this->response->setOutput(json_encode($json));
            ...
            public function save_used_image_to_server() {
                  if (isset($this->request->get['ftp_image'])) {
                      $image = $this->request->get['ftp_image'];
                  } else {
                      $image = false;
                  }
                  ...
                  if ($image) {   
                    // create dir
                    if (!file_exists(DIR_IMAGE . 'catalog/ftp-img')) {
                       mkdir(DIR_IMAGE . 'catalog/ftp-img', 0777, true);
                    }
             

                    //Get the file
                    $image_name = substr(strrchr(rtrim($image, '/'), '/'), 1);
                    $content = file_get_contents($image);
                    //Store in the filesystem.
                    $fp = fopen(DIR_IMAGE . 'catalog/ftp-img/' . $image_name, 'w');
                    fwrite($fp, $content);
                    fclose($fp);

                    $path = "catalog/ftp-img/" . $image_name;
                  } 

                  $this->response->setOutput($path);
              }
              public function elfinder_hash_path($path)
              {
                if ($path == '')
                    $path = DIRECTORY_SEPARATOR;
                $hash = dirname(substr($path, strlen("catalog")+1));
...
if (isset($this->error['name'])) {
		...
		if (isset($this->request->get['order'])) {
            ...
            if (isset($this->request->get['page'])) {
                $drag_url .= '&page=' . $this->request->get['page'];
            }

            if (isset($this->request->get['filter_name'])) {
                $drag_url .= '&filter_name=' . urlencode(html_entity_decode($this->request->get['filter_name'], ENT_QUOTES, 'UTF-8'));
            }

            if (isset($this->request->get['filter_category'])) {
                $drag_url .= '&filter_category=' . urlencode(html_entity_decode($this->request->get['filter_category'], ENT_QUOTES, 'UTF-8'));
            }


            if (isset($this->request->get['filter_model'])) {
                $drag_url .= '&filter_model=' . urlencode(html_entity_decode($this->request->get['filter_model'], ENT_QUOTES, 'UTF-8'));
            }

            if (isset($this->request->get['filter_price'])) {
                $drag_url .= '&filter_price=' . $this->request->get['filter_price'];
            }

            if (isset($this->request->get['filter_quantity'])) {
                $drag_url .= '&filter_quantity=' . $this->request->get['filter_quantity'];
            }

            if (isset($this->request->get['filter_status'])) {
                $drag_url .= '&filter_status=' . $this->request->get['filter_status'];
            }

            if (isset($this->request->get['filter_image'])) {
                $drag_url .= '&filter_image=' . $this->request->get['filter_image'];
            }

            $data['drag_sort_mode_link'] = $this->url->link('catalog/product', 'sort=p.sort_order&order=ASC&drag_sort=1' . $drag_url . '&token=' . $this->session->data['token'], true);

            if (isset($this->request->get['drag_sort']) && $this->request->get['drag_sort']) {
                $data['column_drag_sort'] = '<i class="fa fa-sort" aria-hidden="true"></i>';
            } else {
                $data['column_drag_sort'] = '';
            }

            $data['token'] = $this->session->data['token'];
            $data['min_sort'] = ($product_total) ? ((($page - 1) * $this->config->get('config_limit_admin')) + 1) * 2 - 1 : 0;

            if ($url == '&sort=p.sort_order&order=ASC' && isset($this->request->get['drag_sort']) && $this->request->get['drag_sort']) {
                $url .= '&drag_sort=1';
            }
...
$data['text_tabs'] = $this->language->get('text_tabs');
		...
		$data['text_tab_cross_on'] = $this->language->get('text_tab_cross_on'); 
      	$data['text_tab_cross_off'] = $this->language->get('text_tab_cross_off');  
		$data['text_tab_name'] = $this->language->get('text_tab_name');
		$data['text_add_new_tab'] = $this->language->get('text_add_new_tab');
		$data['text_tab_sort_order'] = $this->language->get('text_tab_sort_order');
		...
		$data['entry_name'] = $this->language->get('entry_name');
              ...
              $data['entry_default_image'] = $this->config->get('implus_default_image_name');
              $data['entry_default_image_selected'] = $this->language->get('entry_default_image_selected');
              ...
              if ($this->config->get('implus_image_folder')) {
                  $data['implus_image_folder'] = $this->config->get('implus_image_folder');
              }
...
$data['attribute_groups'] = $this->model_catalog_attribute_group->getAttributeGroups();
		...
		foreach ($product_options as $product_option) {
			...
			if (isset($product_option['product_option_value'])) {
              ...
              $link_label = "";
              if (!empty($link)) {
                 $link_label = $this->model_catalog_product->getProduct($link);
                 $link_label = $link_label['name'];
              }
		...
		// Image
              ...
              if (isset($this->request->post['image_hash'])) {
                $data['image_hash'] = $this->elfinder_hash_path($this->request->post['image']);
              } elseif (!empty($product_info)) {
                $data['image_hash'] = $this->elfinder_hash_path($product_info['image']);
              } else {
                $data['image_hash'] = '';
              }
		...
		$data['placeholder'] = $this->model_tool_image->resize('no_image.png', 100, 100);
				...
				foreach ($product_videos as $product_video) {
					$data['product_videos'][] = array(
						...
						'preview'  => isset($product_video['preview']) ? $product_video['preview'] : '',
...
if (isset($this->request->post['add_tabs'])) { 
			        $data['tabs_data'] = $this->request->post['add_tabs'];
			    } elseif (isset($this->request->get['product_id'])) {
			        $data['tabs_data'] = $this->model_catalog_product->getAddTabs($this->request->get['product_id']);
			    } else {
			        $data['tabs_data'] = [];
			    }
...
if (isset($this->request->post['add_tabs'])) {
				    foreach ($this->request->post['add_tabs'] as $records) {
				    	if (isset($records['cross'])) {
				    		unset($records['cross']);
				    	}
				        foreach ($records as $language_id => $value) {
				            if (utf8_strlen($value['name']) < 3 || utf8_strlen($value['name']) > 255) {
				                $this->error['name'] = '';
				                break 2;
				            }
		...
		return !$this->error;
          ...
          // pro multiple images
            public function upmulti() {
              if (isset($this->request->get['uimage'])) {
                  $this->load->model('tool/image');
                  $image = $this->model_tool_image->resize(html_entity_decode($this->request->get['uimage'], ENT_QUOTES, 'UTF-8'), 100, 100);
