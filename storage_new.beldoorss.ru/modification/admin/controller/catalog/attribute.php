...
class ControllerCatalogAttribute extends Controller {
	...
	protected function getList() {
		...
		if (isset($this->request->get['order'])) {
            ...
            $data['drag_sort_mode_link'] = $this->url->link('catalog/attribute', 'sort=a.sort_order&order=ASC&drag_sort=1' . $drag_url . '&token=' . $this->session->data['token'], true);
            ...
            $data['min_sort'] = ($attribute_total) ? ((($page - 1) * $this->config->get('config_limit_admin')) + 1) * 2 - 1 : 0;

            if ($url == '&sort=a.sort_order&order=ASC' && isset($this->request->get['drag_sort']) && $this->request->get['drag_sort']) {
                $url .= '&drag_sort=1';
            }
