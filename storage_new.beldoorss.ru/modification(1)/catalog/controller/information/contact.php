...
class ControllerInformationContact extends Controller {
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$contact_form = $this->config->get('tlgrm_notification_contact_form');
			...
			if ($send_ntf == 1 && $contact_form == 1) {
				...
				if (strlen($tlgrm_ntf_id) > 0) {
					...
					foreach ($tlgrm_ntf as $key) {
						$tlgrm_ntf[$key] = $this->config->get('tlgrm_notification_contact_'.$key) || 0;
					}
					...
					if ($this->config->get('tlgrm_notification_contact_comment')) {
