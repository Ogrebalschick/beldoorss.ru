...
class ControllerStartupSeoUrl extends Controller {
      ...
      // OCFilter end
		...
		if (isset($this->request->get['_route_'])) {
			...
			$this->request->get['blogpath'] .= '_' . $url[1];
					...
					if ($query->row['query'] && $url[0] != 'newsblog_category_id' && $url[0] != 'newsblog_article_id' && $url[0] != 'information_id' && $url[0] != 'manufacturer_id' && $url[0] 
	...
	public function rewrite($link) {
		...
		foreach ($data as $key => $value) {
			...
			($data['route'] == 'newsblog/article' && $key == 'newsblog_article_id') || ($data['route'] == 'information/information' && $key == 'information_id') || ($data['route'] == 'blog/blog' && $key == 'blog_id'))
			 {
