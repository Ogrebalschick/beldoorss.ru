...
class ControllerExtensionModuleLatest extends Controller {
	public function index($setting) {
		...
		/*mmr*/
					...
					'thumb'       => $image,
					'name'        => $result['name'],
					'description' => $this->config->get('moneymaker2_modules_latest_description') ? utf8_substr(strip_tags(html_entity_decode($result['description'], ENT_QUOTES, 'UTF-8')), 0, $this->config->get($this->config->get('config_theme') . '_product_description_length')) . '..' : '',
					'price'       => $price,
					'special'     => $special,
					'tax'         => $tax,
					'rating'      => $rating,
