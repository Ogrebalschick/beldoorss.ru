...
class ControllerCommonHeader extends Controller {
	public function index() {
       ...
       $FileManagerElfinder = $data['FileManagerElfinder'] = (array)$this->config->get('FileManagerElfinder_setting');
       if ($this->config->get('config_editor_default')) {
        ...
        if (isset($FileManagerElfinder['status']) && $FileManagerElfinder['status']) {
          $data['scripts'] = str_replace(['view/javascript/summernote/opencart.js','view/javascript/ckeditor/ckeditor_init.js'], '', $this->document->getScripts());
        } else {
		...
		if (!isset($this->request->get['token']) || !isset($this->session->data['token']) || ($this->request->get['token'] != $this->session->data['token'])) {
            ...
            if ($_GET['route'] == 'extension/openbay/orderlist') { 
              $data['openbay_page'] = true;
            } else {
              $data['openbay_page'] = false;
            }       
            ...
            $data['implus_status'] = 0; 
              if ($this->config->get('implus_status')) {
                    ...
                    $data['toolbar_image'] = 'color'; 
                    if ($this->config->get('implus_design_status')) {
                         if ($this->config->get('implus_toolbar_image')) {$data['toolbar_image'] = $this->config->get('implus_toolbar_image');}
                    }     
              } 
