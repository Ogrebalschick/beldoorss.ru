...
class ControllerCommonFileManager extends Controller {
    ...
    public function connector() {
      ...
      if (isset($this->request->server['HTTPS']) && (($this->request->server['HTTPS'] == 'on') || ($this->request->server['HTTPS'] == '1'))) { $catalogpath = HTTPS_CATALOG."image/"; } else {  $catalogpath = HTTP_CATALOG."image/";}   
        ...
        // user catalog
        $im_root_folder_user = "";
        $user_catalog = $this->config->get('implus_user_catalog');
        if ($user_catalog) {
          $user_catalog_admin = $this->config->get('implus_user_catalog_admin');
              $current_user_name = $this->user->getUserName();
              $current_user_name_group_id = $this->user->getGroupId();
          if ($user_catalog_admin != $current_user_name_group_id) {
            if ($this->config->get('implus_default_root')) {
                $root_path = $this->config->get('implus_default_root') . $current_user_name. "/";
              } else {
                $root_path = "catalog/" . $current_user_name. "/";
              }
            if (!file_exists(DIR_IMAGE . $current_user_name)) {
                mkdir(DIR_IMAGE . "catalog/" . $current_user_name, 0777, true);
            }
          } else {
            $root_path = "";
          }
        ...
        //
        $double_size = "48";
        if ($this->config->get('implus_double_images')) {
          $double_size = "96";
        }
        ...
        $im_root_folder = "";
        if ($this->config->get('implus_default_root')) {
          $im_root_folder = $this->config->get('implus_default_root');
        }
        ...
        $settings = array(
           ...
           'plugin' => array(
               'Watermark' => array(
                    ...
                    'middle'         => $this->config->get('implus_watermark_middle'),          
               ...
               'AutoResize' => array(
                 ...
                 'Sanitizer' => array(
                   'enable' => $this->config->get('implus_sanitize'), 
                   'targets'  => array('\\','/',':','*','?','"','<','>','|'), // target chars
                   'replace'  => ''.$this->config->get('implus_sanitize_replace').''     // replace to this
                 )
           ),
      ...
      //'netVolumesSessionKey' => 'netVolumes',
          ...
          'roots' => array(
            array(
              ...
              'path'           => DIR_IMAGE . $im_root_folder . $root_path,    // path to images
              'URL'            => $catalogpath. $im_root_folder . $root_path, // URL path
              'fileMode'       => 0777,
              'dirMode'        => 0777,
            ...
            $ftp_settings = $this->config->get('implus_ftp');
            ...
            foreach ($ftp_settings as $ftp) {
              if ($ftp['status']) {
                $ftp_data = array(
                  ...
                  'user'   => $ftp['user'],
                  'pass'   => $ftp['pass'],
                  'port'   => (is_numeric($ftp['port']) ? (int)$ftp['port'] : "21"),
                  'alias'  => $ftp['alias'],
                  'path'   => $ftp['path'], // must be set manually
                  'URL'  => $ftp['url'], // must be set manually
                  'tmbPath'  => '.tmp',
                  'tmbPathMode'  => 777,
                  'tmpPath'  => '/tmp',
                  'tmbURL'   => dirname($_SERVER['PHP_SELF']) . '/.tmp',  // URL for thumbnails directory set in tmbPath. Set it only if you want to store thumbnails outside root directory
                     // BONUS
                  'mode'   => 'passive',
                ...
                array_push($settings["roots"],$ftp_data); 
              }
            }
          }  
