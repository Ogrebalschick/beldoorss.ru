...
class ControllerCommonColumnLeft extends Controller {
	public function index() {
		if (isset($this->request->get['token']) && isset($this->session->data['token']) && ($this->request->get['token'] == $this->session->data['token'])) {
            ...
            if ($this->user->hasPermission('access', 'extension/module/file_manager')) {
                $data['menus'][] = array(
                    'id'       => 'menu-media',
                    'icon'	   => 'fa-picture-o',
			...
			// Catalog
$data['menus'][] = array('id' => 'menu-moneymaker2','icon' => 'fa-star','name' => 'Moneymaker<sup>2</sup>','href' => $this->url->link('extension/theme/moneymaker2', 'token=' . $this->session->data['token'] . '&store_id=0', true), 'children' => array());
			$catalog = array();
...
if ($this->user->hasPermission('access', 'catalog/simplepars')) {
				$catalog[] = array(
					...
					'href'     => $this->url->link('catalog/simplepars', 'token=' . $this->session->data['token'], true),
					'children' => array()		
				);
			}
			...
			*/	
            ...
            if ($this->user->hasPermission('access', 'tool/implus')) {
                $tool[] = array(
                    'name'     => "Image Manager Pro+",
                    'href'     => $this->url->link('tool/implus', 'token=' . $this->session->data['token'], true),
                    'children' => array()       
                );
            }  
