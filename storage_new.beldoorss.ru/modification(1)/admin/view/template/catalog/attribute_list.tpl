  ...
  <div class="page-header">
    <div class="container-fluid">
      <div class="pull-right">
			...
			<a href="index.php?route=catalog/attribute&token=<?php echo $token; ?>" data-toggle="tooltip" title="Выйти из режима" class="btn btn-primary"><i class="fa fa-sign-out"></i></a>
      ...
      <div class="panel-body">
        <form action="<?php echo $delete; ?>" method="post" enctype="multipart/form-data" id="form-attribute">
          <div class="table-responsive">
            ...
            <?php if ($column_drag_sort) { ?>
            <td class="text-right">
                <input type="text" value="<?php echo $attribute['sort_order']; ?>" class="form-control text-right drag-sort-order-input" data-attribute-id="<?php echo $attribute['attribute_id']; ?>" style="width: 100px; margin-left: auto;">

                <input type="hidden" class="drag-sort-order" value="<?php echo $attribute['sort_order']; ?>" name="attribute_sort[<?php echo $attribute['attribute_id']; ?>]">
            </td>
            <?php } else { ?>
                ...
                $(document).ready(function() {
                    ...
                    $('.btn-make-order').click(function(event) {
                        ...
                        $.ajax({
                            ...
                            type: 'post',
                            data: [{name: 'order', value: 'attributes'}],
                            dataType: 'json',
                            beforeSend: function() {
                            },
                            complete: function() {
            ...
            <script>
                function initInputSort(url) {
                    $('.drag-sort-order-input').change(function() {
                        ...
                        updateSort(url, [{name: 'attribute_sort[' + $(this).attr('data-attribute-id') + ']', value: $(this).val()}], function() {
                            $(".drag-sort-table tbody").sortable("destroy");
                            $('.drag-sort-table').load(window.location.href + ' .drag-sort-table > *', function() {
                                initTable(url);
                                initInputSort(url);
