...
class ControllerExtensionModuleTlgrmNotification extends Controller {
	...
	public function messageOrder($arg = '') {
		...
		$tlgrm_ntf_id = $this->config->get('tlgrm_notification_id');
			...
			if ($order_info) {
				...
				foreach ($msg_title as $key => $value) {
					$title_text = $this->language->get("text_$value");
		   			$title_config = $this->config->get('tlgrm_notification_'.$value);
				...
				foreach ($msg_separator as $key => $value) {
					$separator_text = PHP_EOL;
		   			$separator_config = $this->config->get('tlgrm_notification_'.$value);
				...
				foreach ($common_info as $key => $value) {
					if ($this->config->get('tlgrm_notification_'.$value) && isset($order_info[$value])) {
						$title_text = $this->language->get("text_$value");
			   			$title_config = $this->config->get('tlgrm_notification_text_'.$value);
						if (strlen($title_config) > 0) {
							$title_text = $title_config;
						}
						...
						$common_info[$key] = array(
							...
							'sort'	=> $this->config->get('tlgrm_notification_sort_'.$value),
							'value'	=> $temp,
						);
					} else {
				...
				$simple = $this->config->get('tlgrm_notification_simple');
						    ...
						    foreach ($result as $key => $value) {
			    				...
			    				if (isset($simple[$value['id']]) && $simple[$value['id']]) {
			    					$title_text = $value['label'][$lang_code];
						   			$title_config = $this->config->get('tlgrm_notification_text_simple');
						   			$title_config = $title_config[$value['id']];
									if (strlen($title_config) > 0) {
										$title_text = $title_config;
									}
									...
									$sort = $this->config->get('tlgrm_notification_sort_simple');
				...
				foreach ($products as $pk => $product) {
					...
					foreach ($order_product as $key => $value) {
						...
						if ($this->config->get('tlgrm_notification_product_'.$value)) {

							$title_text = $this->language->get("text_product_$value");
				   			$title_config = $this->config->get('tlgrm_notification_text_product_'.$value);
							if (strlen($title_config) > 0) {
								$title_text = $title_config;
							}							
							...
							$order_products[$pk][$key] = array(
								'title' => '<b>'. $title_text .': </b>',
								'sort'	=> $this->config->get('tlgrm_notification_sort_product_'.$value),
								'value'	=> $temp,
							);
						} else {
				...
				if ($this->config->get('tlgrm_notification_field_merger')) {
					foreach ($shipping_info as $key => $value) {
		...
		if (strlen($tlgrm_ntf_id) > 0 && isset($arg['order_status_id']) && !empty($arg['order_status_id'])) {
		    ...
		    if (isset($arg['order_id']) && !empty($arg['order_id'])) {
				...
				if ($order_info) {
					...
					$template = $this->config->get('tlgrm_notification_template_order_status');
