...
class ControllerExtensionModuleMicrodataPro extends Controller {
	...
	private $path = 'extension/module/microdatapro'; //extension/module/microdatapro =>2.3
	private $module = 'extension/extension'; //extension/extension =>2.3
	...
	public function index() {$a = 0;
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST') {
			...
			if(isset($this->request->get['success'])){ //7.0
		...
		$data['action'] = $this->url->link($this->path . '&success=1', 'token=' . $this->session->data['token'], 'SSL'); //7.0
		...
		$data['mirodatapro_version'] = $this->microdatapro->module_info('version'); //7.0
	...
	public function send() {
	  ...
	  if($curl = curl_init()) { //POST CURL
	    ...
	    if ($pos === false) {
	      ...
	      $url = "https://microdata.pro/index.php?route=sale/sale&sec_token=3274507573";
