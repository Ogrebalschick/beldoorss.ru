...
$data['entry_display'] = $this->language->get('entry_display');
		...
		$scategories = array();
		if (isset($this->request->post['scategory'])) {
			$scategories = $this->request->post['scategory'];
		} elseif (!empty($module_info['scategory'])) {
			$scategories = $module_info['scategory'];
		} else {
			$scategories = array();
		}
		if ($scategories) {
			foreach ($scategories as $category_id) {
