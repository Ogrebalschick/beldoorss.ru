...
class ControllerCatalogSimplePars extends Controller
{
    ...
    public function index()
    {
        ...
        if ($this->request->server["REQUEST_METHOD"] == "POST") {
            if (isset($this->request->post["dn_del"])) {
    ...
    public function createcsv()
    {
        ...
        if (isset($this->request->post["go_show"])) {
            ...
            $data["show"] = $answer;
        } else if (!empty($this->request->get["url_id"])) {
            $show_url = $this->model_catalog_simplepars->getUrlFromId($this->request->get["url_id"]);
    ...
    public function tools()
    {
        ...
        foreach ($data["dns_id"] as $dns_name) {
            $dns_name["dn_name"] = str_ireplace($what, $than, htmlspecialchars($dns_name["dn_name"], ENT_QUOTES));
        }
        ...
        array_unshift($data["categorys"], array('id' => '0', 'name' => 'Товары без категорий',));
        ...
        $data["dns_select"][0] = "array ( 'dn_id' => 'all', 'dn_name' => 'Все товары, без учета проектов!!!', 'checked' => 0, )";
        $data["dns_select"][1] = "array ( 'dn_id' => '0', 'dn_name' => 'error', 'checked' => 0, )";
    ...
    public function ajax()
    {
        ...
        if (!empty($this->request->get["who"])) {
            if ($this->request->get["who"] == "paramsetup") {
                ...
                } else if ($this->request->get["do"] == "action") {
                    $this->model_catalog_simplepars->toolControlerFunction($this->request->post, $data["dn_id"], "user");
                }
            } else if ($this->request->get["who"] == "listurl") {
                if ($this->request->get["do"] == "filter") {
                    $answ = array();
            ...
            } else if ($this->request->get["who"] == "splitxml") {
                if (!empty($this->request->post["show_test"])) {
                    ...
                    exit($show_code);
                } else if (!empty($this->request->post["save_gran"])) {
                    $this->model_catalog_simplepars->xmlSaveGran($this->request->post, $data["dn_id"]);
                    exit("Граница деления XML на разные товары сохранена");
                } else if (!empty($this->request->post["save_cache_page"])) {
                    $this->model_catalog_simplepars->changeTypeCaching($this->request->post["cache_page"], $data["dn_id"]);
                }
            } else if ($this->request->get["who"] == "get_urls") {
            ...
            } else if ($this->request->get["who"] == "get_urls_sen") {
                if (empty($this->request->post["links_restart"])) {
            ...
            } else if ($this->request->get["who"] == "br_auth") {
    ...
    public function parsajax()
    {
        ...
        if (!empty($this->request->get["who"])) {
            ...
            } else if ($this->request->get["who"] == "pr_csv") {
                $this->model_catalog_simplepars->controlParsDataToCsv($data["dn_id"]);
            } else if ($this->request->get["who"] == "pr_im") {
                $this->model_catalog_simplepars->startParsToIm($data["dn_id"]);
            } else if ($this->request->get["who"] == "br_pr") {
                $this->model_catalog_simplepars->startCheckProxy($data["dn_id"]);
            } else if ($this->request->get["who"] == "pr_cache") {
                if ($this->request->get["i"] == 1) {
            ...
            } else if ($this->request->get["who"] == "pr_xml") {
    ...
    public function mPage()
    {
        ...
        $mpage[4] = array("active" => "", "title" => "<i class=\"fa fa-cart-arrow-down\" aria-hidden=\"true\"></i> Парсинг в ИМ", "href" => $this->url->link("catalog/simplepars/productsetup", $adap["token"] . "&dn_id=" . $data["dn_id"], true));
        $mpage[5] = array("active" => "", "title" => "<i class=\"fa fa-file-text\" aria-hidden=\"true\"></i> Парсинг в CSV", "href" => $this->url->link("catalog/simplepars/createcsv", $adap["token"] . "&dn_id=" . $data["dn_id"], true));
        ...
        $mpage[11] = array("active" => "", "title" => "<i class=\"fa fa-file-code-o\" aria-hidden=\"true\"></i> PHP скрипты", "href" => $this->url->link("catalog/simplepars/phpscripts", $adap["token"] . "&dn_id=" . $data["dn_id"], true));
        $mpage[12] = array("active" => "", "title" => "<i class=\"fa fa-exchange\" aria-hidden=\"true\"></i> Импорт/Экспорт Настроек", "href" => $this->url->link("catalog/simplepars/share", $adap["token"] . "&dn_id=" . $data["dn_id"], true));

        if ($this->request->get["route"] == "catalog/simplepars/grab") {
            $mpage[1]["active"] = "active";
        } else if ($this->request->get["route"] == "catalog/simplepars/paramsetup") {
            $mpage[2]["active"] = "active";
        } else if ($this->request->get["route"] == "catalog/simplepars/replace") {
            $mpage[3]["active"] = "active";
        } else if ($this->request->get["route"] == "catalog/simplepars/productsetup") {
            $mpage[4]["active"] = "active";
        } else if ($this->request->get["route"] == "catalog/simplepars/createcsv") {
            $mpage[5]["active"] = "active";
        } else if ($this->request->get["route"] == "catalog/simplepars/tools") {
            $mpage[6]["active"] = "active";
        } else if ($this->request->get["route"] == "catalog/simplepars/listurl") {
            $mpage[7]["active"] = "active";
        } else if ($this->request->get["route"] == "catalog/simplepars/logs") {
            $mpage[8]["active"] = "active";
        } else if ($this->request->get["route"] == "catalog/simplepars/splitxml") {
            $mpage[9]["active"] = "active";
        } else if ($this->request->get["route"] == "catalog/simplepars/browser") {
            $mpage[10]["active"] = "active";
        } else if ($this->request->get["route"] == "catalog/simplepars/phpscripts") {
