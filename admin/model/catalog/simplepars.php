...
public function ParsLink($data, $setting, $dn_id){
	...
	if ($setting['type_grab'] == 1) {
		...
		#$reg_url = '#<a[^>]+?href=["\']?([^"\'>]+)["\']?#s';
		#$reg_url = '#<a.+?href=["\']?([^"\'>]+)["\']?#s';
		#$reg_url = "#window.location.href = '(.*?)'#s";
		
	}elseif($setting['type_grab'] < 3){
		$reg_url = '#<loc>(.*?)<\/loc>#s';
	}else{
		$reg_url = '#<url>(.*?)</url>#s';
		#$reg_url = '#<product_link>(.*?)<\/product_link>#s';
	}
...
public function createCsv($csv, $setting, $dn_id){
  ...
  foreach($csv['value'] as $csv_data){
	  ...
	  foreach($csv_data as $kay => $csv_var){
	    @$csv_data[$kay] = htmlspecialchars_decode(trim(iconv("UTF-8", $csv_charset, $csv_var)));
	  }
...
public function savePrsetup($data, $dn_id){
	...
	if(empty($data['r_file'])){ $data['r_file'] = 0;	}
	...
	if(empty($data['u_file'])){	$data['u_file'] = 0;	}
  ...
  $this->db->query("UPDATE ". DB_PREFIX ."pars_prsetup SET
  	...
  	file='".$this->db->escape($data['file'])."',
  	file_dir='".$this->db->escape($data['file_dir'])."',
  ...
  $this->db->query("UPDATE `". DB_PREFIX ."pars_setting` SET
  	...
  	u_file='".$this->db->escape($data['u_file'])."',
...
public function getLengthClassId(){
	...
	$length_class_id = $this->db->query("SELECT * FROM ".DB_PREFIX."length_class_description WHERE language_id = ".(int)$language_id);
...
public function getWeightClassId(){
	...
	$weight_class_id = $this->db->query("SELECT * FROM ".DB_PREFIX."weight_class_description WHERE language_id = ".(int)$language_id);
	...
	foreach($imgs_chunk as $chunk){
		...
		foreach($data_img as $key => $img){
			...
			if(!empty($img['img'])){
				...
				//Проверяем нужно ли конвертировать изображения из webp.
				if($browser['webp_conv'] > 0 && $ext == '.webp' && function_exists('imagecreatefromwebp') && function_exists('imagejpeg')){
					...
					if($browser['webp_conv'] == 1){
						...
						#Выбираем имя для фото
						$path_img = $path.'/'.$uder_dir.$name.'.jpg';
						//Проверяем есть ли такое фото. Если да то добавляем цифрув начала имени.
						for($i=1;$i>0;){
							if(file_exists($path_img)){
								$path_img = $path.'/'.$uder_dir.$name.'_'.$i.'.jpg';
								$i++;
							}else{
								$i=0;
							}
						...
						#Выбираем имя для фото
						$path_img = $path.'/'.$uder_dir.$name.'.png';
						//Проверяем есть ли такое фото. Если да то добавляем цифрув начала имени.
						for($i=1;$i>0;){
							if(file_exists($path_img)){
								$path_img = $path.'/'.$uder_dir.$name.'_'.$i.'.png';
								$i++;
							}else{
								$i=0;
							}
...
//Загрузка файлов
public function dwFileToProduct($dn_id, $files, $dir, $browser){

	$data = [];

	//Директория для хранения файлов
	/////////////////////////////////
	if(empty($dir)){ $dir = 'file_dir';}
	$href = [];
	#Удаляем слешы из начала и конца имени директории фото. Подготавливаем директорию под загрузку.
	if($dir[0] == '/'){ $dir = substr($dir, 1);}
	if(substr($dir, -1) == '/'){ $dir = substr($dir, 0, -1); }
	...
	$dir = str_replace('\\', '/', $dir);
	#путь к директории на загрузку.
	$path = DIR_DOWNLOAD.$dir;

	//проверяем существование директории, и если ее нет то создаем.
	if(!is_dir($path)){ mkdir($path, 0755, true); }

	//Собираем массив ссылок на скачивание файлов.
	//делем на 10 потоков, заодно преобразовываем ссылки и заготавливаем два массива для сравнения имен после париснга. 
	$i = 0;
	$n = 0;
	$files_chunk = [];
	$names = [];
	foreach($files as $f){
		...
		//Создаем массив для отправки в cURL, мой вариант фунции array_chunk($files, 10);
		if($n == 10){ $n = 0; $i++; }
		$tmp_url = $this->urlEncoding($f[0]);
		$files_chunk[$i][] = $tmp_url;
		$n++;

		//создаем массив где ключ это ссылка на файл а значение это имя файла
		$names[$tmp_url] = $f[1];

	}

	#$this->wtfarrey($files_chunk);
	#$this->wtfarrey($names);
	#$this->wtfarrey($dir);
	...
	foreach($files_chunk as $chunk){
		...
		foreach($data_file as $file){
			//добавляем в массив имя файла для записи в бд. 
			$file['name'] = $names[$file['url']];

			//Формируем имя файла для сохранения на диск.
			$file_temp = str_replace($search, $replace, urldecode($file['url']));
			$file_temp = preg_replace('#\?(.*)#', '', basename($file_temp));

			//Проверяем есть ли расширение файла. Если нет, добавляем принудительно .txt
			$exec = pathinfo($file_temp);
			$name = $exec['filename'];
			$ext = '.txt';
			if(!empty($exec['extension'])){
				$ext = '.'.$exec['extension'];
			...
			//Оставляем только латиницу. И готовим имя файла для загрузки на сервер с солью.
			$name = $this->symbolToEn($name);
			...
			$name_masck = $name.$ext; #имя файла в базе данных, это масска.

			//Записываем файл на диск.
			file_put_contents($path.'/'.$name_file, $file['file']);

			//записываем данные по загруженным файлам. 
			$data[] = ['filename' => $filename, 'mask' => $name_masck, 'name' => $file['name']];
...
*/
public function prDataRep($str, $rep=[]){

	#Стандартная фунция поиск замены
	if(empty($rep)){

		$str = str_replace('{csvnc}', '', $str);

	}else{
		$str = str_replace($rep[0], $rep[1], $str);
	}


	//Функция рандом
	if(strripos($str, '{rand')!==false){
		if(preg_match('#\{rand\[?(.*?)\]?\}#', $str, $rand)){
			
			//проверяем есть ли у рандома условия.
			if(empty($rand[1])){
				$str = str_replace($rand[0], rand(), $str);
			}else{
				$rand_value = explode('-',$rand[1]);
				if(empty($rand_value[1])){ $rand_value[1] = 0; }

				$str = str_replace($rand[0], rand((int)$rand_value[0], (int)$rand_value[1]), $str);
			}
		}
	}
...
public function preparinDataToStore($data, $dn_id){
	...
	//Ниже вырезаем спец маяки из данных где должно быть одно значение, а использовали повторяющиеся границы парсинга.
	$form['model'] = substr(trim($this->madeLogicalMathem($this->prDataRep($form['model']), 'str')), 0, 64);
	$form['sku'] = substr(trim($this->madeLogicalMathem($this->prDataRep($form['sku']), 'str')), 0, 64);
	$form['name'] = mb_substr(trim($this->madeLogicalMathem($this->prDataRep($form['name'],[['{csvnc}','"'],['','&quot;']]), 'str' )), 0, 255);
	#$form['name'] = $this->prDataRep($form['name'],[['{csvnc}','"'],['','&quot;']]);
	//Работаем над ценой товара
	$form['price'] = $this->prDataRep($form['price']);
	$form['price'] = $this->madeLogicalMathem($form['price'], 'int');

	//работаем с ценой скидки.
	$form['price_spec'] = $this->prDataRep($form['price_spec']);
	$form['price_spec'] = $this->madeLogicalMathem($form['price_spec'], 'int');
	if($form['price_spec'] == $form['price']){ $form['price_spec'] = 0; }

	$form['cost'] = $this->prDataRep($form['cost']);
	$form['cost'] = $this->madeLogicalMathem($form['cost'], 'int');

	$form['quant'] = $this->prDataRep($form['quant']);
	$form['quant'] = $this->madeLogicalMathem($form['quant'], 'int');

	$form['manufac'] = trim($this->prDataRep($form['manufac']));
	$form['manufac'] = $this->madeLogicalMathem($form['manufac'], 'str');

	$form['des'] = trim($this->prDataRep($form['des']));
	$form['des'] = $this->madeLogicalMathem($form['des'], 'str');
	
	#Разное
	$form['upc'] = trim($this->prDataRep($form['upc']));
	$form['upc'] = substr($this->madeLogicalMathem($form['upc'], 'str'), 0, 64);

	$form['ean'] = trim($this->prDataRep($form['ean']));
	$form['ean'] = substr($this->madeLogicalMathem($form['ean'], 'str'), 0, 64);
	
	$form['jan'] = trim($this->prDataRep($form['jan']));
	$form['jan'] = substr($this->madeLogicalMathem($form['jan'], 'str'), 0, 64);
	
	$form['isbn'] = trim($this->prDataRep($form['isbn']));
	$form['isbn'] = substr($this->madeLogicalMathem($form['isbn'], 'str'), 0, 64);
	
	$form['mpn'] = trim($this->prDataRep($form['mpn']));
	$form['mpn'] = substr($this->madeLogicalMathem($form['mpn'], 'str'), 0, 64);
	
	$form['location'] = trim($this->prDataRep($form['location']));
	$form['location'] = substr($this->madeLogicalMathem($form['location'], 'str'), 0, 128);
	
	$form['minimum'] = (int)$this->madeLogicalMathem($this->prDataRep($form['minimum']), 'int'); 
	if(empty($form['minimum'])) {$form['minimum'] = 1;}
	
	$form['subtract'] = (int)$this->madeLogicalMathem($this->prDataRep($form['subtract']), 'int');

	$form['length'] = (float)$this->madeLogicalMathem($this->prDataRep($form['length']), 'int');
	if(empty($form['length'])) {$form['length'] = '0.00';}

	$form['width'] = (float)$this->madeLogicalMathem($this->prDataRep($form['width']), 'int');
	if(empty($form['width'])) {$form['width'] = '0.00';}

	$form['height'] = (float)$this->madeLogicalMathem($this->prDataRep($form['height']), 'int');
	if(empty($form['height'])) {$form['height'] = '0.00';}

	$form['length_class_id'] = trim($this->madeLogicalMathem($this->prDataRep($form['length_class_id']), 'int'));
	
	$form['weight'] = $this->prDataRep($form['weight']);
	$form['weight'] = (float)$this->madeLogicalMathem($form['weight'], 'int');
	if(empty($form['weight'])) {$form['weight'] = '0.00';}

	#$this->wtfarrey($form['weight']);

	$form['weight_class_id'] = (int)$this->madeLogicalMathem($this->prDataRep($form['weight_class_id']), 'int');
	$form['status'] = (int)$this->madeLogicalMathem($this->prDataRep($form['status']), 'int');
	$form['sort_order'] = $this->madeLogicalMathem($this->prDataRep($form['sort_order']), 'str');
	$form['layout_pr'] = (int)$this->madeLogicalMathem($this->prDataRep($form['layout_pr']), 'int');
	$form['layout_cat'] = (int)$this->madeLogicalMathem($this->prDataRep($form['layout_cat']), 'int');

	$form['tags'] = trim($this->madeLogicalMathem($this->prDataRep($form['tags']), 'str'));

	#Товар
	$form['seo_url'] = $this->madeLogicalMathem($this->prDataRep($form['seo_url']), 'str');
	$form['seo_h1'] = $this->madeLogicalMathem($this->prDataRep($form['seo_h1']), 'str');
	$form['seo_title'] = $this->madeLogicalMathem($this->prDataRep($form['seo_title']), 'str');
	$form['seo_desc'] = $this->madeLogicalMathem($this->prDataRep($form['seo_desc']), 'str');
	$form['seo_keyw'] = $this->madeLogicalMathem($this->prDataRep($form['seo_keyw']), 'str');
	#Категории
	$form['cat_seo_url'] = $this->madeLogicalMathem($this->prDataRep($form['cat_seo_url']), 'str');
	$form['cat_seo_h1'] = $this->madeLogicalMathem($this->prDataRep($form['cat_seo_h1']), 'str');
	$form['cat_seo_title'] = $this->madeLogicalMathem($this->prDataRep($form['cat_seo_title']), 'str');
	$form['cat_seo_desc'] = $this->madeLogicalMathem($this->prDataRep($form['cat_seo_desc']), 'str');
	$form['cat_seo_keyw'] = $this->madeLogicalMathem($this->prDataRep($form['cat_seo_keyw']), 'str');
	#Производители
	$form['manuf_seo_url'] = $this->madeLogicalMathem($this->prDataRep($form['manuf_seo_url']), 'str');
	$form['manuf_seo_h1'] = $this->madeLogicalMathem($this->prDataRep($form['manuf_seo_h1']), 'str');
	$form['manuf_seo_title'] = $this->madeLogicalMathem($this->prDataRep($form['manuf_seo_title']), 'str');
	$form['manuf_seo_desc'] = $this->madeLogicalMathem($this->prDataRep($form['manuf_seo_desc']), 'str');
	$form['manuf_seo_keyw'] = $this->madeLogicalMathem($this->prDataRep($form['manuf_seo_keyw']), 'str');
	...
	#$this->wtfarrey($form['file']);
	$form['file'] = $this->madeAttrArrey($form['file']);
	#$this->wtfarrey($form['file']);
	...
	#$this->wtfarrey($form['grans_permit_list']);
	//приготавливаем границу для создание папок под фото товара.
	#$form['img_dir'] = trim($form['img_dir'];
	#$form['img_dir'] = $this->prDataRep($form['img_dir'],[['{csvnc}','{!na!}', ' '], ['/', '/', '_']]);

	$form['img_dir'] = $this->symbolToEn($this->prDataRep(trim($form['img_dir']),[['{csvnc}','{!na!}', ' '], ['/', '/', '_']]));
	$form['img_dir'] = preg_replace('#[^a-zA-Z0-9-_/]#', '', $form['img_dir']);

	$form['file_dir'] = $this->symbolToEn($this->prDataRep(trim($form['file_dir']),[['{csvnc}','{!na!}', ' '], ['/', '/', '_']]));
	$form['file_dir'] = preg_replace('#[^a-zA-Z0-9-_/]#', '', $form['file_dir']);
	//готовим имя фото
	
	$form['img_name'] = $this->symbolToEn($this->prDataRep(trim($form['img_name']),[['{csvnc}','{!na!}', ' '], ['', '', '_']]));
	$form['img_name'] = substr(preg_replace('#[^a-zA-Z0-9-_]#', '', $form['img_name']), 0, 240);
	//приготавливаем границу для создание папок под фото товара.
	$form['des_dir'] = $this->symbolToEn($this->prDataRep(trim($form['des_dir']), [['{csvnc}','{!na!}', ' '], ['/', '/', '_']]));
	$form['des_dir'] = preg_replace('#[^a-zA-Z0-9-_/]#', '', $form['des_dir']);
	$form['script_gran'] = $grans_data;
	...
	if($permit == 1){
		...
		if(1){
			...
			if($pr_id){
				...
				// Файлы
				// 0 - Не обновлять. 
				// 1 - Обновлять с удалением.
				// 2 - Обновлять без удаления
				////////////////////////////////////////////////////
				#$this->wtfarrey($setting);
				if($setting['r_file'] == 1){

					//Качаем и сохраняем файл на диск.
					$files = $this->dwFileToProduct($dn_id, $data['file'], $data['file_dir'], $browser);
					
					if(!empty($files)){ 

						//Название файлов записываем на всех языках.
						$langs = $this->getAllLang(); 

						//производим записи в базу данных
						foreach($files as $file){

							//заносим данные о файле в базу.
							$this->db->query("INSERT INTO `".DB_PREFIX."download` SET `filename` = '".$this->db->escape($file['filename'])."', `mask` = '".$this->db->escape($file['mask'])."', date_added = NOW()");
							...
							foreach($langs as $lang){
								...
								$this->db->query("INSERT INTO `".DB_PREFIX."download_description` SET 
									`download_id` = ".(int)$file_id.", 
									`language_id` = ".(int)$lang['language_id'].", 
									`name` = '".$this->db->escape($file['name'])."'");
							}

							//Записываем информацию о файле в товар.
							$this->db->query("INSERT INTO `".DB_PREFIX."product_to_download` SET `product_id` = ".(int)$pr_id.", `download_id` = ".(int)$file_id);
						}
					}
...
public function updateProduct($data, $link, $setting, $dn_id, $pr_id=0, $browser){
	...
	if($pr_id > 0){
		...
		if($setting['u_file'] == 1){
			...
			//получаем все товары что уже привязаны к товару.
			$files_sql = $this->db->query("SELECT download_id FROM `".DB_PREFIX."product_to_download` WHERE product_id = ".(int)$pr_id)->rows;
			
			//и удаляем все товары что привязаны к товару.
			$this->db->query("DELETE FROM `".DB_PREFIX."product_to_download` WHERE `product_id` = ".(int)$pr_id);

			//Перебираем и удаляем записи и файлы.
			foreach($files_sql as $file_sql){
								
				$this->db->query("DELETE FROM `".DB_PREFIX."download_description` WHERE `download_id` = ".(int)$file_sql['download_id']);
				
				$files_place = $this->db->query("SELECT filename FROM `".DB_PREFIX."download` WHERE download_id = ".(int)$file_sql['download_id'])->row;
				$path_del = DIR_DOWNLOAD.'/'.$files_place['filename'];
				@unlink($path_del);
				
				$this->db->query("DELETE FROM `".DB_PREFIX."download` WHERE `download_id` = ".(int)$file_sql['download_id']);
...
public function madeDebugIfo($data, $url){
	...
	if(!empty($data)){
		...
		$text['pr']['file'] = ['name'=>'Файлы', 'row'=>10, 'text'=>''];
		...
		//Файлы отдельный подход
		if(!empty($data['file'])){

			foreach($data['file'] as $file){
				
				if(!empty($file[0])){
					$text['pr']['file']['text'] .= $file[0].' => '.$file[1].PHP_EOL; #Заглушил временно, можно удалить заглушку 
				}
			}
		}
...
public function addNewLinkList($data, $dn_id){
	...
	//Проверяем что бы имя списка не было пустым.
	if(!empty($data['list_name_new'])){
		...
		$this->session->data['error'] = "Список не создан, нельзя создать список без названия.";
	}
}	
...
public function toolDelProducts($data, $prs_id, $dn_id){
	...
	if($prs_id){
		...
		//получаем все товары что уже привязаны к товару.
		$files = $this->db->query("SELECT download_id FROM ".DB_PREFIX."product_to_download WHERE product_id IN (".$prs_id.")")->rows;
		//Если в товарх есть файлы то делаем удаление. 
		if(!empty($files)){
			//и удаляем все товары что привязаны к товару.
			$this->db->query("DELETE FROM ".DB_PREFIX."product_to_download WHERE product_id IN (".$prs_id.")");
			foreach($files as $file){
				
				$this->db->query("DELETE FROM `".DB_PREFIX."download_description` WHERE `download_id` = ".(int)$file['download_id']);
				$files_place = $this->db->query("SELECT filename FROM `".DB_PREFIX."download` WHERE download_id = ".(int)$file['download_id'])->row;
				$path_del = DIR_DOWNLOAD.'/'.$files_place['filename'];
				@unlink($path_del);
				$this->db->query("DELETE FROM `".DB_PREFIX."download` WHERE `download_id` = ".(int)$file['download_id']);
		...
		foreach ($data['filters'] as $filter) {
			...
			if(empty($filter['value']) && $filter['value'] != '0'){
				...
				if($filter['take_filtr'] == '0'){
					...
					if(strripos($filter['value'], '{sdate')!==false){
						#$this->wtfarrey($sdate);
						$sdate = $this->pregSdata($filter['value']);
						if(!empty($sdate)){
							$filter['value'] = str_replace($sdate[0], $sdate[1], $filter['value']);
						}
					}
...
//обнуление остатков в опциях.
public function toolResetQuanToOption($data, $prs_id, $dn_id){

	//Отправляем запрос на обнуление всех остатков в опциях выбранных товаров.
	$this->db->query("UPDATE `".DB_PREFIX."product_option_value` SET quantity = 0 WHERE `product_id` in (".$prs_id.")");
...
public function toolControlerFunction($data, $dn_id, $who = 'user'){
	...
	if(!empty($data['do_tools'])){
		...
		}elseif($data['do_tools'] == 'reset_opt_quan'){
			
			$this->toolResetQuanToOption($data, $prs_id, $dn_id);
...
public function pregRegRight($data){
	...
	//проверяем нужно ли работать с датой.
	if(strripos($data, '{sdate')!==false){
		#$this->wtfarrey($sdate);
		$sdate = $this->pregSdata($data);
		if(!empty($sdate)){
			$data = str_replace($sdate[0], $sdate[1], $data);
		}
	}
...
//Фунция по приобразованию записи дата в массив с текстом что менять и на какую дату.
//Код фунции говно, после первичной обкатки ОПТИМИЗИРУЙ!!!
public function pregSdata($str){
	$data = [];
	$format = "Y-m-d";
	$hour = 0;
	$sign = '+';
	...
	//работа с датами.
	if(preg_match('#\{sdate(.*?)\}#', $str, $sdate)){
		//Записываем исходный тексто который потом будем менять.
		$data[0] = $sdate[0];

		if($sdate[0] == '{sdate}'){
			$data[0] = '{sdate}';
			$data[1] = date("Y-m-d");
			//если все просто отправляем результат.
			return $data;

		}elseif(preg_match('#\[(.+?)\]#', $sdate[1], $formats)){
			$format = $formats[1];
		}
		
		if(preg_match('#\((.+?)\)#', $sdate[1], $hour)){

			if(!empty($hour[1])){
				
					#$this->wtfarrey($hour[1]);

				//получаем оператор для времени.
				if($hour[1][0] == '+'){
					$sign = '+';
					$hour[1] = substr($hour[1], 1);
				}elseif($hour[1][0] == '-'){
					#$this->wtfarrey($hour[1]);
					$sign = '-';
					$hour[1] = substr($hour[1], 1);
...
public function log($mark, $data, $dn_id){
	...
	//пришле запрос с парсинга ФАЙЛОВ.
	if($mark == 'curlFile'){
		$text .='->[ФАЙЛ] : ';
		...
		if($data['errno'] == 0){
			if($data['http_code']==200){
		  		...
		  		$text .= 'Файл НЕ НАЙДЕН. Ответ сервера ['.$data['http_code'].'] Ссылка | '.htmlspecialchars_decode(trim($data['url'])).PHP_EOL;
		  	}elseif($data['http_code']==400){
		  		$text .= 'Не загружен | Неправильный запрос. Ответ сервера ['.$data['http_code'].'] Ссылка | '.htmlspecialchars_decode(trim($data['url'])).PHP_EOL;
		  	}elseif($data['http_code']==403){
		  		$text .= 'Не загружен | Forbidden, доступ запрещен. Ответ сервера ['.$data['http_code'].'] Ссылка | '.htmlspecialchars_decode(trim($data['url'])).PHP_EOL;
		  	}elseif($data['http_code']==301){
		  		$text .= 'РЕДИРЕКТ | Ваш запрос перенаправлен. Ответ сервера ['.$data['http_code'].'] Ссылка входа - '.htmlspecialchars_decode(trim($data['url']))
		  		.' Адрес куда перенаправлен запрос | '.htmlspecialchars_decode(trim($data['redirect_url'])).PHP_EOL;
		  	}elseif($data['http_code']==302){
		  		...
		  		$text .= 'Не загружен | Locked — целевой ресурс из запроса заблокирован от применения к нему указанного метода. Ответ сервера ['.$data['http_code'].'] Ссылка | '.htmlspecialchars_decode(trim($data['url'])).PHP_EOL;
		  	}elseif($data['http_code']==429){
		  		$text .= 'Не загружен | Страница недоступна, слишком много запросов за короткое время. Ответ сервера ['.$data['http_code'].'] Ссылка | '.htmlspecialchars_decode(trim($data['url'])).PHP_EOL;
		  	}elseif($data['http_code']==426){
		  		$text .= 'Не загружен | Сайт отказывается выполнять запрос с использованием текущего протокола. Ответ сервера ['.$data['http_code'].'] Ссылка | '.htmlspecialchars_decode(trim($data['url'])).PHP_EOL;
		  	}elseif($data['http_code']==500){
		  		$text .= 'Не загружен | Сайт донор не может обработать запрос (Internal Server Error, внутренняя ошибка сервера). Ответ сервера ['.$data['http_code'].'] Ссылка | '.htmlspecialchars_decode(trim($data['url'])).PHP_EOL;
		  	}elseif($data['http_code']==502){
		  		$text .= 'Не загружен | Донор получил не верны ответ от вышестоящего сервера. Ответ сервера ['.$data['http_code'].'] Ссылка | '.htmlspecialchars_decode(trim($data['url'])).PHP_EOL;
		  	}elseif($data['http_code']==503){
		  		$text .= 'Не загружен | Сайт донор недоступен на данный момент. Ответ сервера ['.$data['http_code'].'] Ссылка | '.htmlspecialchars_decode(trim($data['url'])).PHP_EOL;
		  	}elseif($data['http_code']==504){
		  		$text .= 'Не загружен | Внутренняя ошибка донора (Gateway Timeout), не получен своевременный ответ. Ответ сервера ['.$data['http_code'].'] Ссылка | '.htmlspecialchars_decode(trim($data['url'])).PHP_EOL;
		  	}elseif($data['http_code']==505){
		  		$text .= 'Не загружен | HTTP-версия, используемая в запроcе, не поддерживается донором. Ответ сервера ['.$data['http_code'].'] Ссылка | '.htmlspecialchars_decode(trim($data['url'])).PHP_EOL;
		  	}else{
	  			$text .= 'НЕИЗВЕСТНЫЙ ОТВЕТ Ответ сервера не распознан. Код ответа ['.$data['http_code'].'] Ссылка | '.htmlspecialchars_decode(trim($data['url'])).PHP_EOL;
		  	}
  		}else{
  			$text .= 'НЕ ЗАГРУЖЕН Код ответа ['.$data['errno'].'] Сообшение = ['.$data['errmsg'].'] Ссылка | '.htmlspecialchars_decode(trim($data['url'])).PHP_EOL;
  		}
  		#Записываем, или дозаписываем данные в лог фаил
		file_put_contents($path, $text, FILE_APPEND);
	}
...
public function xmlMadeCatTree($cat_data, $cat_delim, $i=0, $categories=[], $parent_id = 0, $parent_name = ''){
  ...
  if($i != 0){
  	...
  	//пересобираем массив для боле сложных операций.
  	$temp_cats = [];
  	foreach($cat_data as $row){
  		$temp_cats[$row['category_id']] = $row;
  	}
  	#$this->wtfarrey($cat_data);
  	#$this->wtfarrey($temp_cats);
    $category_data = array();
    foreach ($cat_data as $row) {

      	if (!array_key_exists($row['parent_id'], $temp_cats)) { 
      		$category_data[0][$row['category_id']] = $row;
      	}else{
      		$category_data[$row['parent_id']][$row['category_id']] = $row;
      	}
...
public function multiCurl1($urls, $dn_id, $browser=[]){
	$this->fastscr('1');
...
public function multiCurl($urls, $dn_id, $browser=[]){
	...
	//проверяем xml парсим или нет
	if(stristr($urls[0], '/simplepars/xml_page/') === FALSE) {
		...
		foreach ($urls as $key => $link) {
			$datac[$link] = $this->getXmlPageFromFile($link, $browser, $dn_id);
			if(!empty($datac[$link])){ 
				unset($urls[$key]);
			}else{
				unset($datac[$link]);
...
//Фунция скачивания img
public function curlFile($urls,  $browser=[], $dn_id){
	...
	$files = [];
  	//по очереди обрабатываем каждый ответ из мулти запроса.
	foreach ($datas as $key => $data) {
  		#$this->wtfarrey($data['errno']);
  		#$this->wtfarrey($data['errmsg']);
  		#$this->wtfarrey($data['http_code']);

  		//пишем логи.
		$this->log('curlFile', $data, $dn_id);
   		//если получи по фото что то больше 302 ответа, не записываем это в массив
		if($data['http_code'] > 302){ continue; }
...
public function findCharsetSite($data, $dn_id){
	...
	if($data['size_download'] < 1024000){
	  ...
	  if(empty($charset)){
		...
		$charset = str_ireplace('"', '', $charset);
		#$this->wtfarrey($charset);
		#$data['content'] = preg_replace('#<category id="(\d+)"></category>#', '<category id="\1">\1</category>', $data['content']);
		//Перекодируем страницу в UTF-8
	  	$data['content'] = @mb_convert_encoding($data['content'], "UTF-8", $charset);
	  	#$data['content'] = preg_replace_callback('/\\\\u([0-9a-f]{4})/i', array($this, 'unicode_decode'), $data['content']);
  	}
...
//данная фунция проверяет есть ли закешированная страница, если да то возврашает ее. Если нет отдает false
public function getXmlPageFromFile($link, $browser, $dn_id){
	//составляем путь к предполагаемому файлу кеша.
	$file = DIR_APPLICATION.strstr($link, "simplepars/xml_page/");
	$this->fastscr('до файла');
	//проверяем есть ли такой файл.
	if (file_exists($file)) {
	$this->fastscr('после');
...
public function importFrom($data, $dn_id){
	...
	if(is_array($data)){
		...
		if(!empty($data['setting'])){
			...
			if(empty($data['setting']['r_file'])){ $data['setting']['r_file'] = 0; }
			...
			if(empty($data['setting']['u_file'])){ $data['setting']['u_file'] = 0;}
			...
			$this->db->query("UPDATE `".DB_PREFIX."pars_setting` SET
				...
				`u_file`='".$this->db->escape($data['setting']['u_file'])."',
				...
				`r_file`='".$this->db->escape($data['setting']['r_file'])."',
		...
		if(!empty($data['prsetup'])){
			...
			if(empty($data['prsetup']['file'])){ $data['prsetup']['file'] = ''; }
			if(empty($data['prsetup']['file_dir'])){ $data['prsetup']['file_dir'] = 'product'; }
			...
			$this->db->query("UPDATE `".DB_PREFIX."pars_prsetup` SET
				...
				`file`='".$this->db->escape($data['prsetup']['file'])."',
				`file_dir`='".$this->db->escape($data['prsetup']['file_dir'])."',
