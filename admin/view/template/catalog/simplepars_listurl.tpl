            ...
            <div class="tab-content">
                ...
                <div class="tab-content">
                  <div class="row">
                    <div class="col-sm-3">
                      ...
                      <div class="checkselect">
                        <?php foreach ($list_names as $list_name) {?>
                          <label>
                           <input type="checkbox" id="checkbox_list_name" name="list_name[]" value="<?php echo $list_name['id'] ?>"><?php echo $list_name['name']?>
                          </label>
                        <?php }?>
                      </div>
                    </div>
                    <div class="col-sm-2">
                      ...
                      <div class="checkselect">
                        <?php foreach ($list_errors as $key_e => $list_error) {?>
                          <label>
                            <input type="checkbox" id="checkbox_list_error" name="list_error[]" value="<?php echo $list_error?>"><?php echo $list_error?>
                          </label>
                        <?php }?>
                      </div>
                    </div>
              ...
              <div role="tabpanel" class="tab-pane" id="plist">
                ...
                <div class="tab-content">
                  ...
                  <div class="row">
                    <div class="col-sm-3">
                      <input class="form-control" value="<?php echo $list_name['name']?>" type="text" readonly>
                    </div>
                    <div class="col-sm-3">
                      <button type="submit" name="list_del" value="<?php echo $list_name['id']?>" class="btn btn-danger" data-original-title="Добавить новый список">
                    ...
                    <div class="form-group required">
                      ...
                      <div class="col-sm-2">
                        ...
                        <select name="link_list" id="link_list" class="form-control input-sm">
                          ...
                          <?php foreach ($list_names as $list_name) {?>
                          <option value="<?php echo $list_name['id']?>" <?php echo ($setting['link_list'] == $list_name['id'])?'selected':''?>><?php echo $list_name['name']?></option>
                          <?php }?>
                        </select>
                      </div>
                      <div class="col-sm-2">
                        ...
                        <select name="link_error" id="link_error" class="form-control input-sm">
                          <option value="">Все ссылки</option>
                          <?php foreach ($list_errors as $list_error) {?>
                          <option value="<?php echo $list_error?>" <?php echo ($setting['link_error'] == $list_error)?'selected':''?>><?php echo $list_error?></option>
                          <?php }?>
                        </select>
                      </div>
                      <div class="col-sm-2">
                  ...
                  <div class="well">
                    <div class="row">
                      ...
                      <div class="col-sm-2">
                        ...
                        <input class="form-control" id="pars_pause" type="text" value="<?php echo $setting['pars_pause']?>">
                      </div>
                      <div class="col-sm-4">
                ...
                <div class="tab-content">
                  <div class="row">
                    <div class="col-sm-6 col-lg-6">
                      ...
                      <button name="download_url_price" value="1" type="submit" form="form" data-toggle="tooltip" class="btn btn-default" data-original-title="Скачать прайс с ссылками" <?php echo ($button_url_price)?'':'disabled'?>>
                        <i class="fa fa-download"></i> Скачать прайс с ссылками
                      </button>
                      <button name="del_url_price" type="submit" value="1" form="form" data-toggle="tooltip" class="btn btn-danger" data-original-title="Удалить прайс ссылками" <?php echo ($button_url_price)?'':'disabled'?>>
...
<script type="text/javascript">
  ...
  function get_urls(data){
    //console.log(data);
    $.ajax({
      url: 'index.php?route=catalog/simplepars/ajax&<?php echo $adap['token']?>&who=listurl&do=filter&dn_id=<?php echo $dn_id?>',
      type: "POST",
      data: data,
      dataType: "html",
  ...
  function do_action(data){
    //console.log(data);
    $.ajax({
      url: 'index.php?route=catalog/simplepars/ajax&<?php echo $adap['token']?>&who=listurl&do=action&dn_id=<?php echo $dn_id?>',
      type: "POST",
      data: data,
      dataType: "html",
      success: function (data) {
  ...
  function render_products_table(answ){
    ...
    var url_csv = "<?php echo $url_csv?>";
    var url_im = "<?php echo $url_im?>";
...
<script type="text/javascript">
  $("#do_action").bind("change", function () {
    ...
    if(do_action == 'url_change_list') {
      ...
      <?php foreach ($list_names as $list_name) {?>
        html += '   <option value="<?php echo $list_name["id"]?>"><?php echo $list_name["name"]?></option>';
      <?php }?>
      html += ' </select>';
      html += '</div>';
      ...
      <?php foreach ($list_errors as $list_error) {?>
        <?php if($list_error != '0'){?>
          html += '   <option value="<?php echo $list_error?>"><?php echo $list_error?></option>';
        <?php }?>
      <?php }?>
      html += ' </select>';
      html += '</div>';
...
<script type="text/javascript">
  $(document).ready(function () {
    ...
    function ajaxpars(time1, iter, form) {
      iter++;
      $.ajax({
        url: 'index.php?route=catalog/simplepars/parsajax&<?php echo $adap['token']?>&who=pr_cache&i='+iter+'&dn_id=<?php echo $dn_id?>',
        type: "POST",
        data: form,
        dataType: "html",
        success: function (data) {
