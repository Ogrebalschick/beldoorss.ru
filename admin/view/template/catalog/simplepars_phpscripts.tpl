  ...
  <div class="container-fluid">
    ...
    <div class="panel panel-default">
      <div class="panel-heading">
        <h3 class="panel-title"><i class="fa fa-pencil"></i> Работа с PHP скриптами для <?php echo '<b>['.$setting['dn_name'].']</b>'?></h3>
      </div>
                  ...
                  <div class="panel panel-default">
                    ...
                    <div class="panel-body">
                      ...
                      <?php foreach($scripts as $script_name){ ?>
                        <option id="<?php echo $script_name?>" value="<?php echo $script_name?>" class="btn btn-default btn-sm btn-block"><?php echo $script_name?></option>
                      <?php }?>
                      </select>
                    </div>
                  </div>
                  ...
                  <div class="panel-body">
                    ...
                    <label class="control-label">Использовать скрипты ?</label>
                      <select name="scripts_permit" class="form-control">
                        <option value="1" <?php echo ($setting['scripts_permit']==1)?'selected':''?>>Да</option>
                        <option value="0" <?php echo ($setting['scripts_permit']==0)?'selected':''?>>Нет</option>
                      </select>
                    </div>
                    ...
                    <div id="scripts_list">
                      <?php foreach($tasks as $ts_key => $task){?>
                      <div class="row" id="script_row-<?php echo $ts_key?>">
                        <div class="col-sm-1">
                          ...
                          <select name="scripts_list[<?php echo $ts_key?>][status]" class="form-control" form="form">
                            <option value="1" <?php echo ($task['status']==1)?'selected':''?>>Вкл</option>
                            <option value="0" <?php echo ($task['status']==0)?'selected':''?>>Выкл</option>
                          </select>
                        </div>
                        <div class="col-sm-2">
                          <label class="control-label">Скрипт <span data-toggle="tooltip" data-original-title="Выберите какой скрипт исполнять."></label>
                          <select name="scripts_list[<?php echo $ts_key?>][name]" class="form-control" form="form">
                            <option id="0" value="" class="btn btn-warning btn-sm btn-block">Выберите скрипт</option>
                            <?php foreach($scripts as $script_name){ ?>
                              <option value="<?php echo $script_name?>" <?php echo ($task['name']==$script_name)?'selected':''?>><?php echo $script_name?></option>
                            <?php }?>
                          </select>
                        </div>
                        <div class="col-sm-2">
                          ...
                          <select name="scripts_list[<?php echo $ts_key?>][when_do]" class="form-control" form="form">
                            <option value="0" <?php echo ($task['when_do']==0)?'selected':''?>>Никогда</option>
                            <option value="1" <?php echo ($task['when_do']==1)?'selected':''?>>Перед отправкой ссылки на парсинг</option>
                            <option value="2" <?php echo ($task['when_do']==2)?'selected':''?>>Перед Парсинг в ИМ</option>
                            <option value="3" <?php echo ($task['when_do']==3)?'selected':''?>>После Парсинга в ИМ</option>
                            <option value="4" <?php echo ($task['when_do']==4)?'selected':''?>>Перед Парсинг в CSV</option>
                            <option value="5" <?php echo ($task['when_do']==5)?'selected':''?>>После Парсинг в CSV</option>
                            <!--option value="6" <?php echo ($task['when_do']==6)?'selected':''?>>По завершению парсинга всех ссылок</option-->
                          </select>
                        </div>
                        <div class="col-sm-1">
                          ...
                          <input name="scripts_list[<?php echo $ts_key?>][sort]" type="text" value="<?php echo $task['status']?>" class="form-control" form="form">
                        </div>
                        <div class="col-sm-5">
                          ...
                          <input name="scripts_list[<?php echo $ts_key?>][comment]" type="text" value="<?php echo $task['comment']?>" class="form-control" form="form">
                        </div>
                        <div class="col-sm-1">
                          <button type="button" onclick="$('#script_row-<?php echo $ts_key?>').remove()" class="btn btn-danger" style="margin-top: 20px" title="Удалить"><i class="fa fa-trash-o"></i></button>
                        </div>
                      </div>
                      <?php }?>
                    </div>
...
<script type="text/javascript">
  $(document).ready(function () {
    ...
    function send_request(value) {
      var retVal;
      $.ajax({
        url: 'index.php?route=catalog/simplepars/ajax&who=phpscripts&<?php echo $adap['token']?>&dn_id=<?php echo $dn_id?>',
        type: "POST",
        data: value,
        async: false,
        dataType: "html",
...
<script type="text/javascript">
  var key_script = <?php echo $js_ts_key?>;
  ...
  function addParamPermit(){
    ...
    html +='      <option id="0" value="" class="btn btn-warning btn-sm btn-block">Выберите скрипт</option>';
                  <?php foreach($scripts as $script_name){ ?>
    html +='        <option id="<?php echo $script_name?>" value="<?php echo $script_name?>" class="btn btn-default btn-sm btn-block"><?php echo $script_name?></option>';
                  <?php }?>
    html +='    </select>';
    html +='  </div>';
    html +='  <div class="col-sm-2">';
