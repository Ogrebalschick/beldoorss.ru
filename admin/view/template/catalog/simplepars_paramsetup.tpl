          ...
          <div class="tab-pane active">
            ...
            <div class="well">
                ...
                <div class="col-md-6">
                  <form class="form-horizontal" id="form" method="POST">
                    ...
                    <div class="input-group">
                      <input id="param_name" type="text" name="param_name" class="form-control" value="<?php echo @$activ_param['name']?>">
                    ...
                    <hr />
                    <input type="hidden" id="param_id" name="param_id" value="<?php echo @$activ_param['id']?>">
                    <label for="basic-url">Текст начала парсинга</label>
                    <input id="param_start" type="text" name="param_start" class="form-control" value="<?php echo @$activ_param['start']?>">
                    <hr>
                    <label>Текст конца парсинга</label>
                    <input type="text" id="param_stop" name="param_stop" class="form-control" value="<?php echo @$activ_param['stop']?>">
                    <hr>
                    <div class="row">
                  ...
                  <form class="form-horizontal" id="form_preview" method="POST">
                      <div class="col-md-3">
                        ...
                        <select name="pre_view_param" id="pre_view_param" class="form-control">
                          <option value="1" <?php echo ($setting['pre_view_param']==1)?'selected':''?>>Да</option>
                          <option value="0" <?php echo ($setting['pre_view_param']==0)?'selected':''?>>Нет</option>
                        </select>
                      </div>
                      <div class="col-md-3">
                        <label>Подсветка синтаксиса</label>
                        <select name="pre_view_syntax" id="pre_view_syntax" class="form-control">
                          <option value="1" <?php echo ($setting['pre_view_syntax']==1)?'selected':''?>>Да</option>
                          <option value="0" <?php echo ($setting['pre_view_syntax']==0)?'selected':''?>>Нет</option>
                        </select>
                      </div>
                    </div>
                    <hr>
                  ...
                  <form class="form-horizontal" name="myForm" method="POST">
                    ...
                    <div class="input-group">
                      <select id="get_param_id" name="get_param_id" class="form-control">
                        ...
                        <option value="<?php echo $param['id']?>" <?php if(!empty($activ_param['id']) && $activ_param['id']==$param['id']){echo 'selected' ;}?>>
                          <?php echo ($param['type']==1) ? $param['name'].' | {gran_'.$param['id'].'}' : '@ '.$param['name'].' |{gran_'.$param['id'].'}'?>
                        </option>
                        <?php }?>
                      </select>
                    ...
                    <select id="type_param" name="type_param" class="form-control" form="form">
                      ...
                      <option value="2" <?php echo ($menup['type_param'] == 2 || !empty($activ_param) && $activ_param['type']==2) ? 'selected' : '' ?>>@ Повторяющаясь граница парсина</option>
                    </select>
                    <hr>
                    <div class="row">
                      <div class="col-sm-4 col-md-4">
                        ...
                        <select id="with_teg" name="with_teg" class="form-control" form="form">
                          <option value="1" <?php echo (isset($activ_param['with_teg']) && $activ_param['with_teg'] == 1)?'selected':''?>>Нет</option>
                          <option value="0" <?php echo (isset($activ_param['with_teg']) && $activ_param['with_teg'] == 0)?'selected':''?>>Да</option>
                        </select>
                      </div>
                      <div class="col-sm-4 col-md-4">
                        <label>Пропустить вхождения</label>
                        <input id="skip_enter" name="skip_enter" class="form-control" type="text" value="<?php echo (empty($activ_param['skip_enter']))?0:$activ_param['skip_enter']?>" form="form">
                      </div>
                      <div class="col-sm-4 col-md-4">
                        <label>Где пропускать</label>
                        <select id="skip_where" name="skip_where" class="form-control" form="form">
                          <option value="1" <?php echo (!empty($activ_param['skip_where']) && $activ_param['skip_where'] == 1)?'selected':''?>>С начала</option>
                          <option value="2" <?php echo (!empty($activ_param['skip_where']) && $activ_param['skip_where'] == 2)?'selected':''?>>С конца</option>
                          <option value="3" <?php echo (!empty($activ_param['skip_where']) && $activ_param['skip_where'] == 3)?'selected':''?>>С начала и с конца</option>
                        </select>
                      </div>
                    </div>
                    <hr />
                    <div id="type_hidden" class="row" hidden>
                      <div class="col-sm-6 col-md-4">
                        ...
                        <select id="base_id" name="base_id" class="form-control" form="form">
                          ...
                          <?php foreach($params as $param){
                            if($param['type']==1){ ?>
                              <option <?php if(!empty($activ_param['base_id']) && $activ_param['base_id']==$param['id']){echo 'selected' ;}?>
                            value="<?php echo $param['id']?>"><?php echo $param['name']?></option>
                          <?php }}?>
                        </select>
                      </div>
                      <div class="col-sm-3 col-md-4">
                        <label>Разделитель</label>
                        <input id="delim" name="delim" class="form-control" type="text" value="<?php echo (empty($activ_param['delim']))?'{csvnc}':$activ_param['delim']?>" form="form">
                      </div>
                      <div class="col-sm-3 col-md-4">
                        <label>Порядок вывода</label>
                        <select id="reverse" name="reverse" class="form-control" form="form">
                          <option value="0" <?php echo (!empty($activ_param['reverse']) && $activ_param['reverse'] == 0)?'selected':''?>>Нормальный</option>
                          <option value="1" <?php echo (!empty($activ_param['reverse']) && $activ_param['reverse'] == 1)?'selected':''?>>Обратный</option>
                        </select>
                      </div>
                    </div>
                    <hr>
              ...
              <div class="row">
                <form class="form-horizontal" method="POST">
                  ...
                  <div class="col-sm-7">
                    <input type="text" id="view_href" name="view_href" class="form-control" value="<?php echo $view_link?>" placeholder="Вставьте ссылку на страницу сайта, что бы получить код">
                  </div>
                  <div class="col-sm-1">
                    <?php if (!empty($view_link)) {?>
                    <a href="<?php echo $view_link?>" target="_blank" class="btn btn-default"><i class="fa fa-link"></i></a>
                    <?php }?>
                  </div>
                </form>
            ...
            <div class="row">
              <div class="col-md-10">
                ...
                <?php if($setting['pre_view_syntax']){?>
                <script src="https://cdn.rawgit.com/google/code-prettify/master/loader/run_prettify.js"></script>
                <?php }?>
                <!--Подсветка синтаксиса-->
                <pre id="pre_code" style="max-height: <?php echo ($setting['pre_view_param']==1)?'500px;':'1000px;'?> overflow: auto; padding: 5px" title="Code block"><code id="page_code" class="<?php echo ($setting['pre_view_syntax'])?'prettyprint lang-html linenums':''?>"><?php echo $page_code?></code><code id="piece_code" hidden></code></pre>
                
                <iframe id="iframe" style="<?php echo ($setting['pre_view_param']==1)?'display:on':'display:none';?>" width="100%" height="700" class="" src="" name="iframe" scrolling="auto" allowfullscreen></iframe>

              </div>
              <div class="col-md-2">
                <div class="panel panel-default" style="max-height: 1158px; overflow: auto;">
                  <ul class="list-unstyled" style="white-space: nowrap;">
                    <?php  foreach($hrefs as $key => $href){ ?>
                    <li>
                      <span>[<a title="Открыть страницу в новом окне браузера" target="_blank" href="<?php echo $href['link']?>"><?php echo $key?></a>] </span>
                      <span><a title="Просмотр кода этой страницы" href="<?php echo $view_href.$href['id']?>"><?php echo $href['link']?></a>
                      </span>
                    </li>
                    <?php }?>
                  </ul>
...
<script>
    $(document).ready(function () {
      ...
      if($("#pre_view_param").val() == 1){
        $('iframe').attr('src', "<?php echo $view_link?>");
      }
      ...
      function ajaxPut(value) {
        $.ajax({
          url: 'index.php?route=catalog/simplepars/ajax&who=paramsetup&<?php echo $adap['token']?>&dn_id=<?php echo $dn_id?>',
          type: "POST",
          data: value,
          dataType: "html",
