...
<div id="content">
  ...
  <div class="container-fluid">
    ...
    <div class="alert alert alert-success" hidden id="msg">
      <i class="fa fa-info-circle"></i><?php echo ' Сканирование завершено'?>
    </div>
            ...
            <div class="tab-pane active">
              ...
              <div class="form-group">
                <div class="col-sm-2">
                  ...
                  <input name="dn_name" type="text" value="<?php echo @$setting['dn_name']?>" id="input-pr"
                ...
                <div class="col-sm-1">
                  ...
                  <input name="pars_pause" type="text" min="0" value="<?php echo @$setting['pars_pause']?>"
                ...
                <div class="col-sm-3">
                  ...
                  <input name="start_link" type="text" value="<?php echo @$setting['start_link']?>"
                ...
                <div class="col-sm-2">
                  ...
                  <select class="form-control" name="page_cou_link">
                    <option value="10000000" <?php echo @($setting['page_cou_link']==10000000)?'selected':''?>>Показывать все ссылки</option>
                    <option value="100" <?php echo @($setting['page_cou_link']==100)?'selected':''?>>Показывать 100 ссылок</option>
                    <option value="500" <?php echo @($setting['page_cou_link']==500)?'selected':''?>>Показывать 500 ссылок</option>
                    <option value="1000" <?php echo @($setting['page_cou_link']==1000)?'selected':''?>>Показывать 1000 ссылок</option>
                    <option value="2000" <?php echo @($setting['page_cou_link']==2000)?'selected':''?>>Показывать 2000 ссылок</option>
                    <option value="3000" <?php echo @($setting['page_cou_link']==3000)?'selected':''?>>Показывать 3000 ссылок</option>
                    <option value="4000" <?php echo @($setting['page_cou_link']==4000)?'selected':''?>>Показывать 4000 ссылок</option>
                    <option value="5000" <?php echo @($setting['page_cou_link']==5000)?'selected':''?>>Показывать 5000 ссылок</option>
                    <option value="10000" <?php echo @($setting['page_cou_link']==10000)?'selected':''?>>Показывать 10 000 ссылок</option>
                    <option value="20000" <?php echo @($setting['page_cou_link']==20000)?'selected':''?>>Показывать 20 000 ссылок</option>
                    <option value="50000" <?php echo @($setting['page_cou_link']==50000)?'selected':''?>>Показывать 50 000 ссылок</option>
                    <option value="100000" <?php echo @($setting['page_cou_link']==100000)?'selected':''?>>Показывать 100 000 ссылок</option>
                    <option value="500000" <?php echo @($setting['page_cou_link']==500000)?'selected':''?>>Показывать 500 000 ссылок</option>
                    <option value="1000000" <?php echo @($setting['page_cou_link']==1000000)?'selected':''?>>Показывать 1 000 000 ссылок</option>
                  </select>
                </div>
                <div class="col-sm-1">
                  ...
                  <select class="form-control" name="type_grab">
                    <option value="1" <?php echo @($setting['type_grab']==1)?'selected':''?>>С сайта [HTML]</option>
                    <option value="2" <?php echo @($setting['type_grab']==2)?'selected':''?>>С sitemap [XML]</option>
                    <option value="3" <?php echo @($setting['type_grab']==3)?'selected':''?>>С sitemap [XML + gz] (Не используются настройки запросов) </option>
                    <option value="4" <?php echo @($setting['type_grab']==4)?'selected':''?>>С XML выгрузки тег ссылки &lt;URL&gt;</option>
                  </select>
                </div>
                <div class="col-sm-2">
              ...
              <div class="form-group">
                <div class="col-sm-9">
                  ...
                  <textarea name="link_round" rows="19" id="link_round" form="form_seve_link_round"
                            class="form-control"><?php echo @$round_link ;?></textarea>

                  <div>
                    <button type="submit" name="seve_links_sen" class="btn btn-success" form="form_seve_link_round" onclick="return confirm('Вы уверены что хотите сохранить ссылки введенные в окне сверху ? Обратите внимание что при выполнении этого действия будут удалены все ссылки с проекта и добавлены заново только те что указаны в форме над кнопкой сохранить. Как правило в форме ввода представлены не все ссылки проекта, а только их часть, равная выбранному параметру &laquo;Сколько показывать ссылок&raquo; Так же данное действие не является регулярным и необходимо крайне редко, если вы не уверены что вам это необходимо, не подтверждайте действие.')">
                    ...
                    <button type="submit" name="del_link_round" id="del_link_round" class="btn btn-danger" form="form_del" onclick="return confirm('Вы уверены что хотите очистить список очереди сканирования ?')">
                      <i class="fa fa-trash" aria-hidden="true"></i> <?php echo 'Очистить список'?>
                    </button>
                    <button type="submit" name="use_filter_round"
                            class="btn btn-primary "><i class="fa fa-filter"
                                                        aria-hidden="true"></i> <?php echo 'Повторить фильтрацию'?>
                    </button>
                    ...
                    <p class="text-right"><span
                              class="text-danger"><?php echo 'Обработано ссылок: ' . $count_finish_scan?></span> |
                      <span
                              class="text-success"><?php echo 'Ссылок в очереди: '.$round_links_prepare?></span></p>
                  </div>
                </div>
                <div class="col-sm-3">
                  ...
                  <label class="float-right" for="filter_round_yes">Или/И <span data-toggle="tooltip"
                          data-original-title="<?php echo 'Если галочка установлена то ссылка должна содержать все элементы фильтра. Если не установлена то хотя бы один из фильтров.'?>"></span>
                    <input type="checkbox" id="filter_round_method" name="filter_round_method" value="and"
                           class="float-right" <?php if($setting['filter_round_method'] == 'and'){echo 'checked="checked"';}else{echo '';}?>
                    >
                  </label>
                  <textarea name="filter_round_yes" rows="3" id="filter_round_yes"
                            class="col-sm-6 form-control"><?php echo @$setting['filter_round_yes']?></textarea>
                  <br>
                  ...
                  <textarea name="filter_round_no" rows="3" id="filter_round_no"
                            class="col-sm-6 form-control"><?php echo @$setting['filter_round_no']?></textarea>
                  <br>
                  <div class="row">
                    <div class="col-sm-6">
                      ...
                      <input name="round_param_start" value="<?php echo @$setting['round_param_start']?>" type="text" class="form-control">
                    </div>
                    <div class="col-sm-6">
                      ...
                      <input name="round_param_stop" value="<?php echo @$setting['round_param_stop']?>" type="text" class="form-control">
                    </div>
                  </div>
                  <div class="row">
                    <div class="col-sm-6">
                      ...
                      <input name="filter_round_depth" value="<?php echo @$setting['filter_round_depth']?>" type="text" class="form-control">
                    </div>
                    <div class="col-sm-6">
                      <label for="filter_round_no">Слеш в конце ссылки</label>
                      <select name="filter_round_slash" class="form-control">
                        <option value="0" <?php echo @($setting['filter_round_slash']==0)?'selected':''?>>Все</option>
                        <option value="1" <?php echo @($setting['filter_round_slash']==1)?'selected':''?>>Сохранять ссылки оканчивающиеся только на /</option>
                        <option value="2" <?php echo @($setting['filter_round_slash']==2)?'selected':''?>>Не сохранять ссылки оканчивающиеся на /</option>
                      </select>
                    </div>
                  </div>
                  <div class="row">
                    <div class="col-sm-6">
                      ...
                      <select name="filter_round_domain" class="form-control">
                        <option value="0" <?php echo @($setting['filter_round_domain']==0)?'selected':''?>>Внутренние и внешние ссылки</option>
                        <option value="1" <?php echo @($setting['filter_round_domain']==1)?'selected':''?>>Только внутренние ссылки</option>
                        <option value="2" <?php echo @($setting['filter_round_domain']==2)?'selected':''?>>Только внешние ссылки</option>
                      </select>
                    </div>
                    ...
                    <div class="col-sm-6">
                      ...
                      <textarea name="filter_round_rules" rows="1" id="filter_round_rules"><?php echo @$setting['filter_round_rules']?></textarea>
                    </div>
              ...
              <div class="form-group">
                <div class="col-sm-9">
                  ...
                  <textarea name="links" rows="19" id="links" form="form_seve_link"
                            class="form-control"><?php echo @$finish_link?></textarea>
                  <div>
                    <button type="submit" name="seve_links" class="btn btn-success" form="form_seve_link" onclick="return confirm('Вы уверены что хотите сохранить ссылки введенные в окне сверху ? Обратите внимание что при выполнении этого действия будут удалены все ссылки с проекта и добавлены заново только те что указаны в форме над кнопкой сохранить. Как правило в форме ввода представлены не все ссылки проекта, а только их часть, равная выбранному параметру &laquo;Сколько показывать ссылок&raquo; Так же данное действие не является регулярным и необходимо крайне редко, если вы не уверены что вам это необходимо, не подтверждайте действие.')">
                      <i class="fa fa-floppy-o" aria-hidden="true"></i> Сохранить ссылки
                    </button>
                    ...
                    <p class="text-right"><?php echo 'Найдено ссылок: '.$links_prepare?></p>
                ...
                <div class="col-sm-3">
                  ...
                  <label class="float-right" for="filter_link_yes">Или/И<span data-toggle="tooltip"
                          data-original-title="<?php echo 'Если галочка установлена то ссылка должна содержать все элементы фильтра. Если не установлена то хотя бы один из фильтров.'?>">
                    </span>
                    <input type="checkbox" id="filter_link_method" name="filter_link_method" value="and" 
                    class="float-right" <?php if($setting['filter_link_method'] == 'and'){echo 'checked="checked"';}?>>

                  </label>
                  <textarea name="filter_link_yes" rows="3" id="filter_link_yes"
                            class="col-sm-6 form-control"><?php echo @$setting['filter_link_yes']?></textarea>
                  <br>
                  <label for="filter_link_no">
                    <?php echo 'Не должно быть в ссылках на товар'?>
                    <span data-toggle="tooltip" data-original-title="<?php echo 'Каждый фильтр записывается с новой строки!'?>"></span>
                  </label>
                  <textarea name="filter_link_no" rows="3" id="filter_link_no"
                            class="col-sm-6 form-control"><?php echo @$setting['filter_link_no']?></textarea>
                  <br>
                  <div class="row">
                    <div class="col-sm-6">
                      <label for="filter_round_no">Текст начала сбора
                      ...
                      <input name="link_param_start" value="<?php echo @$setting['link_param_start']?>" type="text" class="form-control">
                    </div>
                    <div class="col-sm-6">
                      <label for="filter_round_no">Текст конца сбора
                      ...
                      <input name="link_param_stop" value="<?php echo @$setting['link_param_stop']?>" type="text" class="form-control">
                    </div>
                  </div>
                  <div class="row">
                    <div class="col-sm-6">
                      ...
                      <input name="filter_link_depth" value="<?php echo @$setting['filter_link_depth']?>" type="text" class="form-control">
                    </div>
                    <div class="col-sm-6">
                      <label for="filter_round_no">Слеш в конце ссылки
                      </label>
                      <select name="filter_link_slash" class="form-control">
                       <option value="0" <?php echo @($setting['filter_link_slash']==0)?'selected':''?>>Все</option>
                       <option value="1" <?php echo @($setting['filter_link_slash']==1)?'selected':''?>>Сохранять ссылки оканчивающиеся только на /</option>
                       <option value="2" <?php echo @($setting['filter_link_slash']==2)?'selected':''?>>Не сохранять ссылки оканчивающиеся на /</option>
                      </select>
                    </div>
                  </div>
                  <div class="row">
                    <div class="col-sm-6">
                      ...
                      <select name="filter_link_domain" class="form-control">
                        <option value="0" <?php echo @($setting['filter_link_domain']==0)?'selected':''?>>Внутренние и внешние ссылки</option>
                        <option value="1" <?php echo @($setting['filter_link_domain']==1)?'selected':''?>>Только внутренние ссылки</option>
                        <option value="2" <?php echo @($setting['filter_link_domain']==2)?'selected':''?>>Только внешние ссылки</option>
                      </select>
                    </div>

                    <div class="col-sm-6">
                      <label for="filter_link_domain">Поиск замены<span data-toggle="tooltip" data-original-title="Снарядное поля указания правил поиск замены. Обратите внимание что поиск замена применяется к ссылке после прохождения ВСЕХ фильтров, то есть самым последним действием перед записью ссылки в очередь."></span></label>
                      <textarea name="filter_link_rules" rows="1" id="filter_link_rules"><?php echo @$setting['filter_link_rules']?></textarea>
                    </div>
...
<script>
    ...
    $(document).ready(function () {
        ...
        function ajaxpars(value) {
            ...
            $.ajax({
                url: 'index.php?route=catalog/simplepars/parsajax&<?php echo $adap['token']?>&who=grab&i='+iteration+'&dn_id=<?php echo $dn_id?>',
                type: "POST",
                data: value,
                dataType: "html",
