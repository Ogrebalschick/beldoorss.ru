    ...
    <div class="panel panel-default">
      ...
      <div class="panel-body">
        <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form-featuredcarusel" class="form-horizontal">
          ...
          <div class="form-group">
            <label class="col-sm-2 control-label" for="input-class"><?php echo $entry_scategory; ?></label>
            <div class="col-sm-10" id="scategory">
              <input type="text" name="scategory" value="" placeholder="<?php echo $text_categories; ?>" id="input-scategory" class="form-control" />
              <div id="featuredcarusel-scategory" class="well well-sm" style="height: 150px; overflow: auto;">
                <?php foreach ($scategories as $scategory) { ?>
                <div id="featuredcarusel-scategory<?php echo $scategory['category_id']; ?>"><i class="fa fa-minus-circle"></i> <?php echo $scategory['name']; ?>
                  <input type="hidden" name="scategory[]" value="<?php echo $scategory['category_id']; ?>" />
                </div>
                <?php } ?>
              </div>
            </div>
...
$('input[name=\'scategory\']').autocomplete({
  source: function(request, response) {
    $.ajax({
  ...
  select: function(item) {
    $('input[name=\'scategory\']').val('');
    
    $('#featuredcarusel-scategory' + item['value']).remove();
    
    $('#featuredcarusel-scategory').append('<div id="featuredcarusel-scategory' + item['value'] + '"><i class="fa fa-minus-circle"></i> ' + item['label'] + '<input type="hidden" name="scategory[]" value="' + item['value'] + '" /></div>');  
  }
});
$('#featuredcarusel-scategory').delegate('.fa-minus-circle', 'click', function() {
  $(this).parent().remove();
});
